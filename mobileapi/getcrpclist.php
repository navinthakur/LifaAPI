<?php
	...
	$getfaq = mysql_query("select * from crpclawsectionact where crpclanguagetype='$languagetype'");
	$getfaqcount = mysql_num_rows($getfaq);
	if ($getfaqcount > 0) {
		$count = "1";
		while($getfaqrows = mysql_fetch_array($getfaq)){
			
			$faqarray[] = ["sectionid"=>$getfaqrows["crpcsectionid"],"acttile"=>$getfaqrows["crpcacttile"],"actchapter" => $getfaqrows["crpcactchapter"],"sectionchapter" => $getfaqrows["crpcsectionchapter"] ];
