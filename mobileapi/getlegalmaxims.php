<?php
	...
	$total_pages_sql = "select count(*) from tbllegalmaxims";
	...
	$getfaq = mysql_query("select * from tbllegalmaxims limit $offset, $no_of_records_per_page");
	$getfaqcount = mysql_num_rows($getfaq);
	if ($getfaqcount > 0) {
		while($getfaqrows = mysql_fetch_array($getfaq)){
			$faqarray[] = ["legalwordid"=>$getfaqrows["legalmaximsid"],"alphabettype"=>$getfaqrows["alphabettype"],"wordtitle"=>$getfaqrows["wordtitle"],"worddescription"=> $getfaqrows["worddescription"]];
		}
