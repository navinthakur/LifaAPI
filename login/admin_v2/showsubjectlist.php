...
<?php
  if (isset($_GET["delsubjectid"])) {
    ...
    if ($delq) {
      echo '<script>window.location = "showsubjectlist.php?courseid='.$courseid.'&semesterid='.$semesterid.'&txtDisplay='.$txtDisplay.'&subjectid='.$delsubjectid.'&status=1&msg=Details Deleted Successfully!!"</script>';
    }else{
      echo '<script>window.location = "showsubjectlist.php?courseid='.$courseid.'&semesterid='.$semesterid.'&txtDisplay='.$txtDisplay.'&subjectid='.$delsubjectid.'&status=0&msg=Sorry Something went Wrong!!"</script>';
    }
  }
...
<?php 
    if (isset($_POST["btnsubmitstream"])) {
      ...
      if ($subjectquery) {
        echo '<script>window.location = "showsubjectlist.php?courseid='.$courseid.'&semesterid='.$semesterid.'&txtDisplay='.$txtDisplay.'&status=1&msg=Details Saved Successfully!!"</script>';
      }else
      {
        echo '<script>window.location = "showsubjectlist.php?courseid='.$courseid.'&semesterid='.$semesterid.'&txtDisplay='.$txtDisplay.'&status=0&msg=Sorry Something went Wrong!!"</script>';
      }
            ...
            <div class="widget-box">
              ...
              <?php
                ...
                if (isset($_POST["btnUpdatestream"])) {
                  ...
                  if ($updatequery) {
                   echo '<script>window.location = "showsubjectlist.php?courseid='.$courseid.'&semesterid='.$semesterid.'&txtDisplay='.$txtDisplay.'&status=1&msg=Details Deleted Successfully!!"</script>'; 
                  }else{
                    echo '<script>window.location = "showsubjectlist.php?courseid='.$courseid.'&semesterid='.$semesterid.'&txtDisplay='.$txtDisplay.'&status=0&msg=Sorry Something went Wrong!!"</script>';
                  }
                }
              ...
              <a href="showsemesterlist.php?courseid=<?php echo $_GET["courseid"]?>" class="btn btn-success btn-md">Go Back</a>
              ...
              <div class="row-fluid">
                ...
                <div class="row-fluid">
                <div class="span3">
                    <a href="showsyllabuslist.php?courseid=<?php echo $_GET["courseid"]?>&semesterid=<?php echo $_GET["semesterid"]?>&txtDisplay=<?php echo $_GET["txtDisplay"]?>&subjectid=<?php echo $getfaqrows["subjectid"]?>">
                      <div class="stat-block">
                        <ul>
                          <li class="stat-count" style="width: 100%;">
                            <span style="font-size: 13px"><?php echo $getfaqrows["subjectname"]?> </span>
                            <a href="showsubjectlist.php?courseid=<?php echo $_GET["courseid"]?>&semesterid=<?php echo $_GET["semesterid"]?>&txtDisplay=<?php echo $_GET["txtDisplay"]?>&delsubjectid=<?php echo $getfaqrows["subjectid"]?>" class="pull-right"> <span class="black-icons trashcan "></span></a>

                            <a href="showsubjectlist.php?courseid=<?php echo $_GET["courseid"]?>&semesterid=<?php echo $_GET["semesterid"]?>&txtDisplay=<?php echo $_GET["txtDisplay"]?>&viewsubjectid=<?php echo $getfaqrows["subjectid"]?>"> <span class="black-icons pencil pull-right"></span></a>
                          </li>                      
                        </ul>
                      </div>
                    </a>
