...
<?php
  ...
  $clatcontent = $getclataboutrows["documentcontent"];

  if (isset($_POST["BtnSubmitDetails"])) {
    $clatcontent = $_POST["clatcontent"];
    $update = mysql_query("UPDATE `tblcollegedoc` SET `documentcontent`='$clatcontent'");
    if ($update) {
      echo '<script>window.location = "college_doc.php?status=1&msg=Details Saved Successfully!!"</script>';
    }else{
      echo '<script>window.location = "college_doc.php?status=0&msg=Sorry Something went Wrong!!"</script>';
    }
  }
?>
