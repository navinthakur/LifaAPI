...
<div id="main-content">
  <div class="container-fluid">
    <ul class="breadcrumb">
      ...
      <li class="">COLLEGE</li><span class="divider">&raquo;</span>
...
<?php
  $getclatabout = mysql_query("select * from tblcollegetimetable");
  $getclataboutrows = mysql_fetch_array($getclatabout);
  $clatcontent = $getclataboutrows["timetablecontent"];

  if (isset($_POST["BtnSubmitDetails"])) {
    $clatcontent = $_POST["clatcontent"];
    $update = mysql_query("UPDATE `tblcollegetimetable` SET `timetablecontent`='$clatcontent'");
    if ($update) {
      echo '<script>window.location = "college_timetable.php?status=1&msg=Details Saved Successfully!!"</script>';
    }else{
      echo '<script>window.location = "college_timetable.php?status=0&msg=Sorry Something went Wrong!!"</script>';
    }
  }
?>
