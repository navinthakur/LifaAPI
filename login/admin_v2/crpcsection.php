    ...
    if (isset($_POST["btnsubmitchapterdetails"])) {
       ...
       for($i = 0; $i < $chapteritemcount; $i++){
          $actuniqueid = mt_rand(10000, 99999);
            $PropertyQuery = mysql_query("INSERT INTO `crpclawsectionact`(`crpclanguagetype`,`crpcactuniqueid`,`crpcacttile`, `crpcactchapter`, `crpcsectionchapter`) VALUES ('$languagetype','$actuniqueid','$chapterintro[$i]','$actchapter[$i]','( $chapterstartfrom[$i] to $chapterendfrom[$i] )')");
       }
       if ($PropertyQuery) {
       // $msg =  '<label class="label label-success">Details Saved Successfully!!</label><br><br>';
        echo '<script>window.location = "crpcsection.php?current=crpcsection&status=1&msg=Details Saved Successfully!!"</script>';

      }else{
        //$msg = '<label class="label label-danger">Sorry Something Went Wrong!!</label><br><br>';

        echo '<script>window.location = "crpcsection.php?current=crpcsection&status=0&msg=Sorry Something went Wrong!!"</script>';
      }
    ...
    if (isset($_POST["BtnSubmitDetails"])) {
      ...
      $ChapterExplanation = $_POST["ChapterExplanation"];
       ...
       for($i = 0; $i < $itemcount; $i++){
           $PropertyQuery = mysql_query("INSERT INTO `crpcsectionactintro`(`crpcsectionlanguagetype`,`crpcsectionactno`,`crpcsectionacttitle`, `crpcactintro`, `crpcsectionrefid`) VALUES ('$sectionlanguagetype','$Sectionno[$i]','$ChapterTitle[$i]','$ChapterExplanation[$i]','$selectedappuniqueid')");
        }
                          ...
                          <div class="span12">
                            <div class="span3">
                              <div class="control-group">
                                ...
                                <?php
                                    $getchapter = mysql_query("SELECT * FROM `crpclawsectionact`");
                                    while($getchapterrows = mysql_fetch_array($getchapter)){

                                  ?>
                                  <option value="<?php echo $getchapterrows["crpcactuniqueid"]?>"><?php echo $getchapterrows["crpcactchapter"]?> <?php echo $getchapterrows["crpcsectionchapter"]?></option>
                                  <?php
                                      }
                                  ?>
                              </select>
