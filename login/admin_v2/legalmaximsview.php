...
<?php
  if (isset($_GET["view"])) {
    ...
    $getdetails = mysql_query("select * from tbllegalmaxims where legalmaximsid='$legalwordid'");
...
<?php
    ...
    if (isset($_POST["BtnSubmitDetailsLegalTerms"])) {
      ...
      $PropertyQuery = mysql_query("UPDATE `tbllegalmaxims` SET `alphabettype`='$legalwordsalphabet',`wordlanguagetype`='$languagetype',`wordtitle`='$ChapterTitle',`worddescription`='$ChapterExplanation' WHERE `legalmaximsid`='$legalwordid'");
       
      
        if ($PropertyQuery) {
         // $msg =  '<label class="label label-success">Details Saved Successfully!!</label><br><br>';
          echo '<script>window.location = "legalmaximsview.php?view='.$legalwordid.'&status=1&msg=Details Updated Successfully!!"</script>';

        }else{
          //$msg = '<label class="label label-danger">Sorry Something Went Wrong!!</label><br><br>';

          echo '<script>window.location = "legalmaximsview.php?view='.$legalwordid.'&status=0&msg=Sorry Something went Wrong!!"</script>';
        }
