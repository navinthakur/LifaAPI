      ...
      <div class="span12">
        ...
        <div class="nonboxy-widget">
          ...
          <div class="widget-content">
            ...
            <?php
              $tablename = $getfaqrows["subcategorytablename"];
              $subcategorypagename = $getfaqrows["subcategorypagename"];
              $getclatabout = mysql_query("select * from $tablename");
              $getclataboutrows = mysql_fetch_array($getclatabout);
              $clatcontent = $getclataboutrows["content"];

              if (isset($_POST["BtnSubmitDetails"])) {
                $categoryid = $_POST["categoryid"];
                $subcategoryid = $_POST["subcategoryid"];
                $clatcontent = $_POST["clatcontent"];
                $update = mysql_query("UPDATE `$tablename` SET `content`='$clatcontent'");
                if ($update) {
                  echo '<script>window.location = "'.$subcategorypagename.'?categoryid='.$categoryid.'&subcategoryid='.$subcategoryid.'&status=1&msg=Details Saved Successfully!!"</script>';
                }else{
                  echo '<script>window.location = "'.$subcategorypagename.'?categoryid='.$categoryid.'&subcategoryid='.$subcategoryid.'&status=0&msg=Sorry Something went Wrong!!"</script>';
                }
              }
            ?>
