...
<?php
  $getclatabout = mysql_query("select * from tblcollegenotice");
  $getclataboutrows = mysql_fetch_array($getclatabout);
  $clatcontent = $getclataboutrows["noticecontent"];

  if (isset($_POST["BtnSubmitDetails"])) {
    $clatcontent = $_POST["clatcontent"];
    $update = mysql_query("UPDATE `tblcollegenotice` SET `noticecontent`='$clatcontent'");
    if ($update) {
      echo '<script>window.location = "college_notice.php?status=1&msg=Details Saved Successfully!!"</script>';
    }else{
      echo '<script>window.location = "college_notice.php?status=0&msg=Sorry Something went Wrong!!"</script>';
    }
  }
?>
